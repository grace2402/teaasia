...
class GWStatusChecker:
    ...
    def check_pids(self, pids: List[str], include_devices: bool = True,
                   parallel: bool = True, max_workers: Optional[int] = None) -> List[GWCheckResult]:
        """
        批量檢查多個 PID，回傳所有結果。

        Args:
            pids: PID 列表
            include_devices: 是否同時檢查 Device
            parallel: 若 True 使用 ThreadPoolExecutor 平行執行（預設）
            max_workers: 覆蓋 self.max_workers 的執行緒數

        Returns:
            List[GWCheckResult] — 按輸入 pids 順序回傳
        """
        if not parallel or len(pids) <= 1:
            results = []
            for pid in pids:
                try:
                    results.append(self.check_pid(pid, include_devices))
                except Exception as e:
                    err_result = GWCheckResult(pid=pid, error=str(e))
                    results.append(err_result)
            return results

        workers = max_workers or self.max_workers
        results_map = {}  # pid -> GWCheckResult

        with ThreadPoolExecutor(max_workers=workers) as pool:
            future_to_pid = {pool.submit(self.check_pid, pid, include_devices): pid
                             for pid in pids}
            ...
            for future in as_completed(future_to_pid):
                ...
                try:
                    ...
                    # 單一 PID 失敗不影響其他，回傳 error result
                    err_result = GWCheckResult(pid=pid, error=str(e))
                    results_map[pid] = err_result

        # 按原始順序回傳
        return [results_map.get(pid, GWCheckResult(pid=pid, error='unknown')) for pid in pids]
